...
if uploaded_file:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["نظرة عامة", " تحليلات بصرية"," البيانات المفقودة"," عرض البيانات","الجنسيات"])
    ...
    # ---------------- Tab 5: توزيع الموظفين جغرافياً ---------------- #
    with st.expander(" توزيع الموظفين حسب الدولة (خريطة)"):
        st.markdown("###  خريطة توضح عدد الموظفين حسب الجنسية")
        ...
        if 'الجنسية' in df.columns:
            ...
            fig_map = px.choropleth(
                ...
                color="عدد الموظفين",
                color_continuous_scale=['#C8D9E6', '#2F4156'],
                title="توزيع الموظفين حسب الجنسية"
        )
            fig_map.update_layout(title_x=0.5)
            st.plotly_chart(fig_map, use_container_width=True)

        else:
            st.warning(" لا يوجد عمود 'الجنسية' في البيانات.")

    # ---------------- Tab 5: الموقع الجغرافي ---------------- #
    tab5 = st.tabs([" الموقع الجغرافي"])[0]
